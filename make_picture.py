...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='pavia', choices=['paviaU', 'trento','pavia'])
    ...
    parser.add_argument('--patch', type=int, default=13, choices=[7, 9, 11, 13, 15])
...
def dataload(args):
    ...
    del test_data
    del test_label
    test_known_loader, test_loader, target_da_metatrain_data, indices = get_target_dataset(
    ...
    del train_data
    return train_loader, test_known_loader, test_loader, GroundTruth, indices
...
def test(args, model, test_known_loader, test_loader):
    ...
    return prediction, support_label
...
def main():
    ...
    train_loader, test_known_loader, test_loader, GroundTruth, indices = dataload(args)
    model = FeatureNet(args.n_ways, args.feature_dim, data_info)
    model.cuda()
    train(args, model, train_loader)
    prediction, support_label = test(args, model, test_known_loader, test_loader)
    get_cls_map.get_cls_map(prediction, support_label, GroundTruth, indices)
